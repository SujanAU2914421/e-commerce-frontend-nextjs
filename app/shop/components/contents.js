...
export default function Contents({
	...
	useEffect(() => {
		setFilteredData(
			allProducts?.filter((item) => shopCurrentCategory === "all" || item.category.slug === shopCurrentCategory)
		);
	}, [allProducts]);
	...
	return (
		<div className="relative h-auto w-full pt-4 grid gap-8">
			<div className="relative h-auto w-full pt-4 grid gap-4">
				...
				<div className="relative h-auto w-full flex gap-2 items-center" style={{ fontFamily: "afacad-flux" }}>
					<div className="relative font-medium text-gray-800">Home</div>
					<div className="relative text-gray-500">
						<ChevronRight size={14} />
					</div>
					<div className="relative font-medium text-gray-500">{shopCurrentCategory}</div>
				</div>
					...
					<div className="relative h-auto w-auto">
						<div className="relative h-auto w-auto flex gap-6 text-[0.8rem] text-gray-500 flex-wrap">
							...
							{visibleCategories?.slice(0, 7).map((category, index) => (
								...
								>
									<div className="relative uppercase group-hover:text-black">{category.title}</div>
									<div
										className={`absolute -bottom-1 ${
			...
			{filteredData?.length === 0 ? (
				...
				<ExploreProducts allProducts={filteredData} setCurrentQuickViewProduct={setCurrentQuickViewProduct} />
			)}
		</div>
	);
}
